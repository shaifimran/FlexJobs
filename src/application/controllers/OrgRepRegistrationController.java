...
public class OrgRepRegistrationController {
	...
	private void handleRegisterAction() {
		...
		String organisation = organisationField.getText().trim().toLowerCase();
		String position = positionField.getText().trim();

		// Validate inputs
		if (name.isEmpty() || phone.isEmpty() || password.isEmpty() || email.isEmpty() || organisation.isEmpty()
				|| position.isEmpty()) {
		...
		if (dbHandler.isOrganisationRepExists(organisation, email)) {
			showAlert(Alert.AlertType.ERROR, "Representative Exists",
					"An organization representative already exists for this organization with this email.");
			return;
		}

		if (dbHandler.isOrganisationRepExists(organisation)) {
			OrgRep = new OrganisationRepresentative(name, password, email, position, phone, organisation, true);
		} else {
		...
		if (!dbHandler.isOrganisationExists(organisation)) {
			...
			dbHandler.addNotification(organisation, "admin", "Organisation: " + organisation
					+ " and its Organisation Representative: " + email + " needs verification.");
			return;
		}

		dbHandler.addOrgRepresentative(name, phone, password, email, organisation, position,OrgRep.getIsVerified());
		dbHandler.addNotification(email, organisation,
				"Organisation Representative: " + email + " needs verification.");
		showAlert(Alert.AlertType.INFORMATION, "Registration Successful",
				"Organisation Representative registered successfully.");
		clearFields();
		redirectToLoginPage();
	}
